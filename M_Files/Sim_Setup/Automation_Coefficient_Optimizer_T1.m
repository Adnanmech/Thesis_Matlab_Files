    ...
    %for cntr2=sim_pts:-1:1
       for cntr1=1:sim_pts_la
            for cntr=1:sim_pts_yaw_P
                ...
                min_rms_YE
                if  VMC(4000,16) > 14.5 %...            %make sure velocity is greater than 5m/s by 5s
...
Filename_mat = sprintf('Sliding_Mode_Control_CSA_Test_T1_%s.mat', datestr(now,'mm-dd-yyyy_HH-MM'));
save(Filename_mat);
...
%Save figure
Filename_yrmserr_v_ygain_fig = sprintf('Sliding_Mode_Control_CSA_Test_yrmserr_v_ygain_T1_fig_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));
...
Filename_fig = sprintf('Sliding_Mode_Control_CSA_Test_T1_fig_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));

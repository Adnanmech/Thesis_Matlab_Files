...
stop_time = 6.001;   %simulation run time (HAS TO BE CHANGED HERE AND ALSO IN MODEL FILE)
...
Slip_Err_D_Gain_Lowest_X = repmat(100, 1, nsp);
        ...
        for cntr=sim_pts:-1:1
            for cntr1=sim_pts:-1:1
                ...
                if  VMC(4000,16) > 15 ...            %make sure velocity is greater than 5m/s by 5s
                    && max(VMC(1:5900,16)) >  VMC(6000,16)%...     %make sure Vy lower than 1m/s the entire time.
...
Filename_mat = sprintf('Sliding_Mode_Control_CSA_w200kcnrstiffns_w0YawD_Test_%s.mat', datestr(now,'mm-dd-yyyy_HH-MM'));
save(Filename_mat);
...
Filename_fig = sprintf('Sliding_Mode_Control_CSA_Test_w200kcnrstiffns_w0YawD_fig_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));

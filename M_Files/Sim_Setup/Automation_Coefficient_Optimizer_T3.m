...
stop_time = 7.901;   %simulation run time (HAS TO BE CHANGED HERE AND ALSO IN MODEL FILE)
    ...
    %for cntr1=sim_pts_wa:-1:1
        for cntr2=1:sim_pts_yaw
            %for cntr3=1:sim_pts_dyaw
                ...
                Yaw_Ctrl_Gain = -2 + cntr2*3
                dYaw_Ctrl_Gain = 0.02 %cntr3*0.066 - 0.046
                Slip_Ratio_Ctrl_Gain = 9 %cntr*2 - 1
                Wheel_Accel_Ctrl_Gain = 0.3%cntr1*0.1
                ...
                min_rms_YE
                if  VMC(4000,16) > 15 %...            %make sure velocity is greater than 7m/s by 4s
...
Filename_mat = sprintf('Fuzzy_Control_CSA_Test_175Nm_T3_%s.mat', datestr(now,'mm-dd-yyyy_HH-MM'));
save(Filename_mat);
...
Filename_fig = sprintf('Fuzzy_Control_CSA_Test_175Nm_T3_fig%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));
